...
import type { ProductoAdmin } from "../types/api.types";
...
export default function AdminProducts() {
  const { user } = useAuth();
  const esGeneral = user?.rol === "admin_general";
  ...
  const [form, setForm] = useState({ nombre: "", descripcion: "", grupo_id: "" });

  const fetchData = async () => {
    try {
      const products = await adminService.listarProductos();
  ...
  const handleSave = async () => {
    try {
      const body: { nombre: string; descripcion: string; grupo_id?: string } = {
        nombre: form.nombre,
        descripcion: form.descripcion,
      };
      // Regla D: solo admin_general puede especificar grupo_id; el resto lo infiere el backend
      if (esGeneral) body.grupo_id = form.grupo_id;

      if (editing) {
        await adminService.actualizarProducto(editing._id, body);
      } else {
        await adminService.crearProducto(body);
      }
      setShowForm(false);
      setEditing(null);
      setForm({ nombre: "", descripcion: "", grupo_id: "" });
      await fetchData();
    } catch (err) {
  ...
  const handleEdit = (product: ProductoAdmin) => {
    setEditing(product);
    setForm({ nombre: product.nombre, descripcion: product.descripcion || "", grupo_id: obtenerGrupoId(product.grupo_id) ?? "" });
    setShowForm(true);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setEditing(null); setForm({ nombre: "", descripcion: "", grupo_id: "" }); setShowForm(true); }}
          ...
          <div key={product._id} className="bg-card rounded-3xl border border-border p-5 hover:shadow-sm transition-shadow">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-medium text-muted-foreground">
                {obtenerNombreGrupo(product.grupo_id) ?? (typeof product.grupo_id === "string" ? product.grupo_id : "Sin grupo")}
              </span>
            </div>
          </div>
        ))}
